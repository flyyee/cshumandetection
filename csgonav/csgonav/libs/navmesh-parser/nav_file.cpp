...
namespace nav_mesh {
	...
	std::vector<nav_area> nav_file::get_areas_by_position(vec3_t position) {
		std::vector<nav_area> areaVector;
		for (auto& area : m_areas) {
			if (area.is_within(position)) {
		...
		if (areaVector.size() == 0) {
			throw std::exception("nav_file::get_areas_by_position: failed to find area");
		}
		else {
			return areaVector;
		}
