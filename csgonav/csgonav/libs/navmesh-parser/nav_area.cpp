...
namespace nav_mesh {
	...
	float nav_area::GetZ(float x, float y, bool safety) {
		if (safety) {
			nav_mesh::vec3_t safetyvec = {x, y, 0.0};
			if (!is_within(safetyvec)) {
				throw std::exception("nav_area::GetZ: point given is not within area");
				return 0;
			}
		}
		float width = m_se_corner.x - m_nw_corner.x;
		float height = m_se_corner.y - m_nw_corner.y;
		float distanceFromEast = m_se_corner.x - x;
		float distanceFromSouth = m_se_corner.y - y;
		float northSlope = (m_nw_corner.z - m_ne_z) / width;
		float southSlope = (m_sw_z - m_se_corner.z) / width;
		float northZ = m_ne_z + northSlope * distanceFromEast;
		float southZ = m_se_corner.z + northSlope * distanceFromEast;
		float finalLineSlope = (northZ - southZ) / height;
		float z = southZ + finalLineSlope * distanceFromSouth;
		return z;
	}
